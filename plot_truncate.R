plotor_tr <- function(st, ed, filename, dirname){
  ...
  {
    te1 = -(par[2]*a)^par[1]
    te2 = log(par[1]*par[2])-lgamma(x = 1/par[1])-log(1-pgamma(par[2]^par[1], shape = 1/par[1], scale = 1))
    val=sum(te1)+n*te2
    return(-val)
  }
  ...
  fgamma <- function(s, la, al){
    return(exp(-(la*s)^al)*(la*al)/(gamma(x = 1/al)*(1-pgamma(la^al, shape = 1/al, scale = 1))))
  }
