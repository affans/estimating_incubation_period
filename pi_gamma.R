pi_gamma <- function(st, ed, filename, dirname){
  # revise the dataset
  ...
  for(pai in piset){
    ...
    mt[2, ] = c(alp, gam, mean, ssquar)
    
    # nonparametric_bootstrap
    B = 1000
    matr <- matrix(0, B, 11)
    colnames(matr) <- c("alpha","gamma", "mean", paste(quar*100,"th-", "quantile", sep = ""))
    for(i in 1:B){
      att = sample(dura_left, n, replace = TRUE)
      objet=function(par)
      {
        te=log(1- pgamma(att, shape = par[1], scale = par[2]))
        te1=-log(par[1]*par[2])
        val=sum(te)+n*te1
        return(-val)
      }
