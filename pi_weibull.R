pinonparam <- function(st, ed, filename, dirname){
# revise the dataset
...
obj=function(par)
{
  te1=log(pai*(par[2]*a)^(par[1]-1)+
            (1-pai)/(gamma(x = 1/par[1])))-(par[2]*a)^par[1]
  te2 = log(par[1]*par[2])
  val=sum(te1)+n*te2
  return(-val)
}
...
for(i in 1:B){
  ...
  objet=function(par)
  {
    te1=log(pai*(par[2]*att)^(par[1]-1)+
              (1-pai)/(gamma(x = 1/par[1])))-(par[2]*att)^par[1]
    te2 = log(par[1]*par[2])
    val=sum(te1)+n*te2
    return(-val)
  }
